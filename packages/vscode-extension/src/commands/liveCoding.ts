...
import { getDockerContext, getDockerFilePath, invokeDockerode as $, LOG_OUTPUT, removeHostContainer } from '../utils/utils';
...
export async function liveCoding(node?: IWXContainerId, nodes?: IWXContainerId[], buildImage?: boolean): Promise<void> {
...
async function startOne(progress: vscode.Progress<{ message?: string; increment?: number }>, node?: IWXContainerId, buildImage?: boolean): Promise<void> {
  if (!node) {
...
async function startOneLocal(progress: vscode.Progress<{ message?: string; increment?: number }>, node: IWXContainerId, buildImage?: boolean): Promise<void> {
  ...
  try {
    generateDockerComposeAndDockerfileDev((await getDockerFilePath(local)).uri.fsPath, {
    ...
    vscode.window
      ...
      .then(({ title }) => {
        if (title) {
          vscode.env.openExternal(vscode.Uri.parse('https://developers.weixin.qq.com/miniprogram/dev/wxcloudrun/src/guide/debug/dev.html'));
        }
      });
    LOG_OUTPUT.show();
    return;
  ...
  if (buildImage || !local.container) {
    ...
    (async function () {
      ...
      const openBrowser = await vscode.window.showInformationMessage(`实时开发已启动，访问地址：${url}`, '在浏览器中打开');
      if (openBrowser) {
