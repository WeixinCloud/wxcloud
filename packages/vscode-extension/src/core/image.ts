...
export async function _ensureProxyImage(
...
export async function ensureRemoteProxySetup(
  p: (s: string) => void
...
export async function _ensureRemoteProxySetup(
  p: (s: string) => void
): Promise<IEnsureRemoteProxySetup> {
  // check cloud env
  ...
  if (!envId) {
    ...
    envId = await vscode.window.showQuickPick(
      result.list.map(env => env.envId),
      {
        placeHolder:
          '云环境 ID（本地调试访问 VPC 时将访问指定环境的 VPC）。Cloud EnvID. (will access the corresponding VPC during local debug)'
      }
    );
    if (!envId) {
  ...
  // generate unique otp
  const TOAL_KEY = `TOAL_${Math.random().toString(36).substring(2)}`;
  ...
  for (let i = 0; i < 60; i++) {
    ...
    if (
      queryResult.domainInfo.defaultPublicDomain &&
      queryResult.server.versionItems.some(versionItemPredicator) &&
      queryResult.key
    ) {
...
async function _ensureCode(opt: IGetCodeOptions): Promise<IProxyImageInfo> {
  ...
  const resp = await got.get(
    `https://dldir1.qq.com/WechatWebDev/clouddebugproxy/${version}.tar.gz`,
    {
      agent: getProxyAgent(),
      responseType: 'buffer'
    }
