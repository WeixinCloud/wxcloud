...
export function analyzeDockerfile(dockerfile: string): DevDockerfileElements {
  ...
  console.assert(
    stagesCount <= 2,
    'currently only dockerfiles with two or less stages are supported'
  );
  ...
  console.assert(
    !!runStageElements.entrypoint,
    'the last stage of the dockerfile must contains a CMD or an ENTRYPOINT command'
  );
...
function analyzeCommands(commands: CommandEntry[]): DevDockerfileElements {
  ...
  const necessaryCommandsRaw = commands
    .filter(c => ['COPY', 'ADD', 'RUN', 'WORKDIR'].includes(c.name))
    .map(c => c.raw);
...
function getDockerfileStages(commands: CommandEntry[]) {
  ...
  return values(
    groupBy(commands, ({ lineno }) => findLast(fromLinenos, fromLineno => lineno >= fromLineno))
  );
}

function mergeElements(elements: DevDockerfileElements[]): DevDockerfileElements {
  return elements.reduce(
    (prev, curr) => merge(prev, curr, (a, b) => (isArray(a) ? a.concat(b) : undefined)),
    {}
  ) as DevDockerfileElements;
}
