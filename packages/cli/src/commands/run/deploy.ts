  ...
  async getReleaseConfig(
    ...
    printVerticalTable(
      [
        ...
        {
          部署方式: newReleaseConfig.DeployType === 'package' ? '手动上传代码包' : '镜像拉取'
        },
        {
          发布模式: newReleaseConfig.ReleaseType === 'FULL' ? '全量发布' : '灰度发布'
        },
  ...
  async packageDeploy(releaseConfig) {
    ...
    try {
      return await execWithLoading(
        async () => {
          const { UploadUrl, PackageName, PackageVersion } = await DescribeCloudBaseBuildService({
            EnvId,
