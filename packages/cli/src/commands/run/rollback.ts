...
export default class RunRollbackCommand extends Command {
    ...
    return await execWithLoading(
      async () => {
        ...
        const currentVersion = VersionItems?.find(
          ({ FlowRatio, IsDefaultPriority }) => FlowRatio === 100 || IsDefaultPriority
        );
        const versionList = VersionItems.filter(
          ({ VersionName }) => VersionName !== currentVersion?.VersionName
        )
          .filter(({ Status }) => Status === 'normal')
          .filter(({ VersionName }) => VersionName < (currentVersion?.VersionName || ''));
        return {
  ...
  async run() {
    ...
    const { rollbackVersions, currentVersion } = await this.getRollbackVersions(envId, serviceName);
    const versionName =
      flags.version || (await chooseVersionName({ versionList: rollbackVersions }));
    if (
      flags.noConfirm ||
      (await cli.confirm(
